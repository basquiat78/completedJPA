...
첫 번째 커넥션은 시쿼스를 흉내낸 hibernate_sequence테이블을 위해서 사용했고 두 번째 커넥션은 해당 hibernate_sequence을 셀렉트와 미리 id를 확보할려는 update 시점에서 사용되어졌고 마지막 녀석은 batch를 위해서 사용했다.
...
1이구만!!! 알았어~ 첫 번째 엔티티에는 id값을 1로 세팅해야지. 그전에 다음에 사용할 아이디값을 위해 update를 치자 1증가 시켜서 2로 업데이트하고 첫번째로 영속성 컨텍스트에 올라갈 엔티티 녀석에 1을 세팅하자. 커넥션 하나 사용하고 일단 반환!

자 그럼 다음 엔티티를 세팅해보자. 아차! 그럼 hibernate_sequence조회를 해볼까?

2이구만!!! 알았어~ 두 번째 엔티티에는 id값을 2로 세팅해야지. 그전에 다음에 사용할 아이디값을 위해  update를 치자 1증가 시켜서 3로 업데이트하고 두 번째로 영속성 컨텍스트에 올라갈 엔티티 녀석에 2을 세팅하자. 커넥션 하나 사용하고 일단 반환!

자 그럼 다음 엔티티를 세팅해보자. 아차! 그럼 hibernate_sequence조회를 해볼까?

3이구만!!!! 알았어~ 세 번째 엔티티에는 id값을 3로 세팅해야지. 그전에 다음에 사용할 아이디값을 위해  update를 치자 1증가 시켜서 4로 업데이트하고 세 번째로 영속성 컨텍스트에 올라갈 엔티티 녀석에 3을 세팅하자. 커넥션 하나 사용하고 일단 반환!
...
하지만 코드에는 그런 코드가 어디에도 없다. 단지 나는 셀렉트해 온 엔티티에 그냥 가격을 할인 가격으로 변경했을 뿐이다.
...
하지만 이 flush는 그렇다고 해당 엔티티가 영속성컨텍스트에서도 사라지는 것은 아니다. ~~응가의 향기는 여전히 나를 감싸고~~
